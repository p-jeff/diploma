...
**Audio**: two cleanly separated paths —
- **Poem VO → 2D, at the head.** A single shared `[Poem VO]` AudioSource (child of `[Experience Manager]`, `spatialBlend = 0`) plays the narration. Every plant's `audioSource` field points at it; `Plant.Show()` drives `Play()` with the selected plant's clip. Gating and `EnvironmentMoment` read `plant.AudioSource` (= this shared source).
...
- `Plant.audioSource` points at the shared `[Poem VO]` source (2D, child of `[Experience Manager]`) and plays the poem clip (from `PlantData.audioClip`) on `Show()` — narration at the head, not localised to the plant. `Plant.sfxSource` is the plant's own `[BuildingBlock] Spatial Audio` source (3D) used for feedback SFX.
- Like is gated until `AudioSource.isPlaying` becomes false — you can never skip a poem.
