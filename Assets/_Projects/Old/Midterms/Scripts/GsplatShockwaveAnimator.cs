...
namespace Midterms
{
    ...
    public class GsplatShockwaveAnimator : MonoBehaviour
    {
        ...
        bool m_pendingInitial;

        public void ApplyInitialGreyscale()
        {
            if (!TryGetBlock(out var pb))
            {
                // PropertyBlock isn't ready yet (created on GsplatRenderer's first Update).
                // Set a flag and apply in Update, which works in both edit and play mode.
                m_pendingInitial = true;
                return;
            }
            WriteInitial(pb);
        }
        ...
        void Update()
        {
            if (m_pendingInitial && TryGetBlock(out var pb))
            {
                WriteInitial(pb);
                m_pendingInitial = false;
            }
        }
