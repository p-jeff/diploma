// Based on Gsplat/Animated (MIT License)
// Added: per-splat distance-based scale — splats closer to _GsplatProximityTarget appear bigger

Shader "Gsplat/ProximityScale"
{
    Properties {}
    SubShader
    {
        ...
        Pass
        {
            ...
            // Animation uniforms (from GsplatHueAnimator)
            float _GsplatHueShift;
            float4 _GsplatTintColor;
            float _GsplatOpacityMul;

            // Per-splat proximity scale uniforms (set by GsplatPerSplatProximity.cs)
            float3 _GsplatProximityTarget;        // world-space position of the reference point
            float  _GsplatProximityNearDist;      // distance at which nearDownscale applies
            float  _GsplatProximityFarDist;       // distance at which farDownscale applies
            float  _GsplatProximityNearDownscale; // downscale factor when close (0=full, 1=invisible)
            float  _GsplatProximityFarDownscale;  // downscale factor when far  (0=full, 1=invisible)

            // RGB <-> HSV conversion (carried over from GsplatAnimated)
            ...
            v2f vert(appdata v)
            {
                ...
                // --- Per-splat proximity scaling ---
                // Read local position (buffer reads are GPU-cached from InitSplatData)
                #ifdef UNCOMPRESSED
                float3 _proxLocalPos = _PositionBuffer[source.id];
                #endif
                #ifdef SPARK
                float3 _proxLocalPos, _proxScale; float4 _proxColor, _proxQuat;
                UnpackSplat(_PackedSplatsBuffer[source.id], _proxColor, _proxLocalPos, _proxScale, _proxQuat);
                #endif
                float3 worldPos = mul(_MATRIX_M, float4(_proxLocalPos, 1.0)).xyz;
                float dist = distance(worldPos, _GsplatProximityTarget);
                float range = max(_GsplatProximityFarDist - _GsplatProximityNearDist, 0.0001);
                float t = saturate((dist - _GsplatProximityNearDist) / range);
                float downscale = lerp(_GsplatProximityNearDownscale, _GsplatProximityFarDownscale, t);
                float perSplatScale = 1.0 - downscale;

                // Only scale the screen-space offset, not the UV.
                // Scaling UV would break the fragment falloff formula.
                corner.offset *= perSplatScale;
                // -----------------------------------
